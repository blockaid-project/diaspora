...
SELECT posts.* FROM posts WHERE posts.type = 'Reshare' AND posts.public <> 1; SELECT posts.* FROM posts WHERE FALSE;
SELECT posts.root_guid FROM posts WHERE posts.type = 'Reshare'; SELECT posts.guid FROM posts WHERE posts.public = 1;

-- contact-aspect
SELECT contacts.* FROM contacts, aspects, aspect_memberships WHERE aspect_memberships.aspect_id = aspects.id AND aspect_memberships.contact_id = contacts.id AND contacts.user_id <> aspects.user_id; SELECT contacts.* FROM contacts WHERE FALSE;

-- If I receive a notification on a post, I must be able to view the post.
SELECT notifications.recipient_id, notifications.target_id FROM notifications WHERE notifications.target_type = 'Post'; (SELECT users.id, posts.id FROM users, posts WHERE posts.`public` = 1) UNION (SELECT people.owner_id, posts.id FROM people, posts WHERE posts.author_id = people.id) UNION (SELECT share_visibilities.user_id, posts.id FROM share_visibilities, posts WHERE share_visibilities.shareable_id = posts.id AND share_visibilities.shareable_type = 'Post');
