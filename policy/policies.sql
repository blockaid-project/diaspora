...
-- extended_profiles
SELECT profiles.* FROM profiles WHERE profiles.public_details = 1;
SELECT profiles.* FROM profiles, contacts WHERE contacts.user_id = _MY_UID AND contacts.person_id = profiles.person_id AND contacts.sharing = 1;
...
SELECT aspect_memberships.* FROM aspect_memberships, roles, people WHERE roles.person_id = people.id AND roles.name = 'admin';
